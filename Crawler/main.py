...
def crawler_main():
    ...
    # Stage 2:         
    with open('./Output/url.txt', 'r', encoding='utf8') as input_file:
    ...
    for i in tqdm(range(0, len(data))):
        ...
        for j, post in enumerate(row):
            ...
            try:
                ...
                with open(f'Output/Context_raw/{i}_{j}.txt', 'w+', encoding='utf8') as output_file:
    ...
    for i in tqdm(range(len(content))):
        ...
        for j, post in enumerate(row):
            # start_time = time.time()
            try:
                if exists(join(RAW_DIR, f'{i}_{j}.txt')):
                    with open(join(RAW_DIR, f'{i}_{j}.txt'), 'r', encoding='utf8') as file:
                        url = file.readline()
                        response = file.read()
                ...
                if require_Javascript(context):
                    continue
                with open(f"Output/Context_plain/{i}_{j}.txt", 'w+', encoding='utf8') as file:
