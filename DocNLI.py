...
# with open("./nli_pred_plain.txt", 'w+') as file:
    # json.dump(final_test_performance, file, cls=NpEncoder)

final_test_performance = json.load(open('nli_pred_plain.txt', 'r'))
df = pd.read_json(join(ANNOTATION_DIR, 'test_data.json'), lines=True)
ground_truth = df['context_label'].to_list()
...
pred = [get_mode(li) for li in nli_grouped_by_image_id]
import collections
...
with open("./docnli.txt", 'w+') as file:
